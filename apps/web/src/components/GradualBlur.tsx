...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    // mobile/tablet/desktop overrides are built from the key name (e.g. "mobileHeight"),
    // so there's no static key to point at, hence the loose record
    const dims = config as Record<string, string | undefined>;
    const calc = () => {
      const w = window.innerWidth;
      let v = config[key];
      const cap = (s: string) => s.charAt(0).toUpperCase() + s.slice(1);
      const k = cap(key);
      if (w <= 480 && dims['mobile' + k]) v = dims['mobile' + k];
      else if (w <= 768 && dims['tablet' + k]) v = dims['tablet' + k];
      else if (w <= 1024 && dims['desktop' + k]) v = dims['desktop' + k];
      setVal(v);
    };
...
// presets/curves attached as statics so consumers can reach them off the component
type GradualBlurComponent = typeof GradualBlurMemo & {
  PRESETS: typeof PRESETS;
  CURVE_FUNCTIONS: typeof CURVE_FUNCTIONS;
};
const GradualBlurWithStatics = GradualBlurMemo as GradualBlurComponent;
